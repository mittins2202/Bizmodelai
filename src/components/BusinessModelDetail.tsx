...
import { ArrowLeft, Clock, DollarSign, TrendingUp, Zap, Users, Package, CheckCircle, AlertTriangle, Star, Target, Lightbulb, ArrowRight, FileText, Award } from 'lucide-react';
  ...
  return (
    <div className="relative">
      ...
      <div className="relative bg-white p-6 rounded-xl border border-gray-200">
        ...
        <div className="absolute bottom-2 left-0 text-xs text-gray-500 mt-4">$0</div>
      </div>
    </div>
  );
};
...
const BusinessModelDetail: React.FC<BusinessModelDetailProps> = ({ quizData }) => {
  ...
  const [aiAnalysis, setAiAnalysis] = useState<AIPersonalizedAnalysis | null>(null);
  ...
  const generateAIAnalysis = async (data: QuizData, path: BusinessPath) => {
    try {
      ...
      const { personalizedPros, personalizedCons } = await generatePersonalizedProsAndCons(data, path);
      
      // Generate struggles and solutions
      const strugglesAndSolutions = await generateStrugglesAndSolutions(data, path);
  ...
  const generateFitDescription = async (data: QuizData, path: BusinessPath): Promise<string> => {
...
Write a personalized explanation of why this business model fits their specific profile.`;
    ...
    try {
      const aiService = AIService.getInstance();
      const response = await (aiService as any).makeOpenAIRequest(prompt, 300, 0.7);
  ...
  const generatePersonalizedProsAndCons = async (data: QuizData, path: BusinessPath): Promise<{ personalizedPros: string[]; personalizedCons: string[] }> => {
...
- Con 3`;
    ...
    try {
      const aiService = AIService.getInstance();
      const response = await (aiService as any).makeOpenAIRequest(prompt, 400, 0.7);
  ...
  const generateStrugglesAndSolutions = async (data: QuizData, path: BusinessPath): Promise<string> => {
...
Focus on their specific weaknesses and how to overcome them.`;
  ...
  const generateSkillsAssessment = async (data: QuizData, path: BusinessPath): Promise<{ hasSkills: string[]; needsSkills: string[]; developingSkills: string[] }> => {
    ...
    requiredSkills.forEach(skill => {
      ...
      if (skillLower.includes('communication') && data.directCommunicationEnjoyment >= 4) {
        hasSkills.push(skill);
      } else if (skillLower.includes('creative') && data.creativeWorkEnjoyment >= 4) {
        hasSkills.push(skill);
      } else if (skillLower.includes('tech') && data.techSkillsRating >= 4) {
        hasSkills.push(skill);
      } else if (skillLower.includes('organization') && data.organizationLevel >= 4) {
        hasSkills.push(skill);
  ...
  const parseProsCons = (response: string): { personalizedPros: string[]; personalizedCons: string[] } => {
    ...
    return {
      personalizedPros: pros.length > 0 ? pros : generateFallbackProsCons(businessPath!).personalizedPros,
      personalizedCons: cons.length > 0 ? cons : generateFallbackProsCons(businessPath!).personalizedCons
    };
  };

  const generateFallbackAnalysis = (path: BusinessPath): AIPersonalizedAnalysis => {
    return {
  ...
  const generateFallbackProsCons = (path: BusinessPath): { personalizedPros: string[]; personalizedCons: string[] } => {
    return {
      personalizedPros: [
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black/50 backdrop-blur-sm">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Take the Quiz First</h2>
          <p className="text-gray-600 mb-6">
            To access personalized insights about {businessPath.name}, you need to complete our business path quiz first.
          </p>
  ...
  const userFitScore = personalizedPaths.find(p => p.id === businessPath.id)?.fitScore || 0;
  const getFitLevel = (score: number) => {
    if (score >= 80) return { level: 'Best Fit', color: 'green', description: 'This business model is an excellent match for your profile.' };
    if (score >= 65) return { level: 'Strong Fit', color: 'blue', description: 'This business model aligns well with your goals and preferences.' };
    if (score >= 50) return { level: 'Possible Fit', color: 'yellow', description: 'This business model could work with some adjustments.' };
    return { level: 'Poor Fit', color: 'red', description: 'This business model may not be ideal for your current situation.' };
  };
        ...
        <div className="max-w-6xl mx-auto px-8 py-12">
          ...
          <section id="overview" className="mb-16">
            ...
            <div className="grid lg:grid-cols-3 gap-12 mb-12">
              ...
              <div>
                <h3 className="text-2xl font-bold text-gray-900 mb-4">About {businessPath.name}</h3>
                ...
                <div className="flex items-center">
                  <div className="flex-1 bg-gray-200 rounded-full h-3 mr-4">
                    <div 
                      ...
                      style={{ 
                        width: businessPath.difficulty === 'Easy' ? '90%' : 
                               businessPath.difficulty === 'Medium' ? '70%' : '50%' 
                      }}
                    />
                  </div>
            ...
            <div className="grid md:grid-cols-4 gap-6 mb-12">
              {[
                { icon: Clock, title: 'Time to Start', value: businessPath.timeToProfit, color: 'blue' },
                { icon: DollarSign, title: 'Initial Investment', value: businessPath.startupCost, color: 'green' },
                { icon: TrendingUp, title: 'Potential Income', value: businessPath.potentialIncome, color: 'purple' },
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg mb-12">
              ...
              <div className="prose prose-lg text-gray-700">
                <p>This business model is ideal for {businessPath.bestFitPersonality?.join(', ').toLowerCase()} individuals who are looking to build a sustainable income stream.</p>
              </div>
            </div>
          ...
          <section id="personalized-report" className="mb-16">
            <h2 className="text-3xl font-bold text-gray-900 mb-8">How This Business Model Fits You</h2>

            {/* Fit Assessment */}
            <div className="bg-white rounded-2xl p-8 shadow-lg mb-8">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Is this model a good fit for you?</h3>
              <div className="flex items-center mb-4">
                <div className={`px-4 py-2 rounded-full text-sm font-bold ${
                  fitLevel.color === 'green' ? 'bg-green-100 text-green-800' :
                  fitLevel.color === 'blue' ? 'bg-blue-100 text-blue-800' :
            ...
            {isGeneratingAI ? (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-lg mb-8">
                <h3 className="text-xl font-bold text-gray-900 mb-4">Why We Chose This For You</h3>
                <div className="prose prose-lg text-gray-700">
            ...
            {aiAnalysis && (
              <div className="bg-yellow-50 rounded-2xl p-8 mb-8">
                <h3 className="text-2xl font-bold text-gray-900 mb-6">Things You'll Struggle With</h3>
                <div className="prose prose-lg text-gray-700">
            ...
            {aiAnalysis && (
              <div className="bg-white rounded-2xl p-8 shadow-lg mb-8">
                <h3 className="text-2xl font-bold text-gray-900 mb-8">Required Skills Assessment</h3>
                <div className="grid md:grid-cols-3 gap-8">
                  <div>
                    <h4 className="font-semibold text-green-800 mb-4">✅ Skills You Have</h4>
                    <div className="flex flex-wrap gap-2">
                      {aiAnalysis.skillsAssessment.hasSkills.map((skill, index) => (
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {aiAnalysis.skillsAssessment.developingSkills.map((skill, index) => (
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {aiAnalysis.skillsAssessment.needsSkills.map((skill, index) => (
          ...
          <section id="get-started" className="mb-16">
            ...
            <p className="text-xl text-gray-600 mb-8">
              Now that you've decided on {businessPath.name}, here are your next steps!
            </p>

            {/* Step-by-Step Launch Plan */}
            <div className="bg-white rounded-2xl p-8 shadow-lg mb-8">
              <h3 className="text-2xl font-bold text-gray-900 mb-6">Step-by-Step Launch Plan</h3>
            ...
            {/* Resources */}
            <div className="bg-white rounded-2xl p-8 shadow-lg mb-8">
              <h3 className="text-2xl font-bold text-gray-900 mb-6">Resources to Get Started</h3>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-semibold text-green-800 mb-4">🆓 Free Resources</h4>
                  <ul className="space-y-2">
                    {businessPath.resources.learning.filter(resource => resource.includes('Free') || resource.includes('YouTube')).map((resource, index) => (
                      <li key={index} className="text-gray-700">• {resource}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h4 className="font-semibold text-blue-800 mb-4">💰 Paid Resources</h4>
                  <ul className="space-y-2">
                    {businessPath.resources.learning.filter(resource => !resource.includes('Free') && !resource.includes('YouTube')).map((resource, index) => (
                      <li key={index} className="text-gray-700">• {resource}</li>
                    ))}
            ...
            <div className="bg-red-50 rounded-2xl p-8 mb-8">
              <h3 className="text-2xl font-bold text-gray-900 mb-6">Mistakes to Avoid</h3>
              <ul className="space-y-3">
            ...
            <div className="bg-gradient-to-r from-green-600 to-blue-600 rounded-2xl p-8 text-center text-white">
              <div className="flex items-center justify-center mb-4">
                ...
                <h3 className="text-2xl font-bold">Good Luck on Your Journey to Wealth!</h3>
              </div>
              <p className="text-lg opacity-90">
                You have everything you need to succeed. Take action, stay consistent, and watch your business grow!
              </p>
            </div>
          </section>
        </div>
