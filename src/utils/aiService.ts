...
export class AIService {
  ...
  async generatePersonalizedInsights(quizData: QuizData, topPaths: BusinessPath[]): Promise<{
    ...
    try {
      ...
      const personalizedSummary = await this.generatePersonalizedSummary(userProfile, topBusinessPaths);
      
      // Generate custom recommendations
      const customRecommendations = await this.generateCustomRecommendations(userProfile, topBusinessPaths);
      
      // Generate potential challenges
      const potentialChallenges = await this.generatePotentialChallenges(userProfile, topBusinessPaths);
      
      // Generate success strategies
      const successStrategies = await this.generateSuccessStrategies(userProfile, topBusinessPaths);
      
      // Generate personalized action plan
      const personalizedActionPlan = await this.generatePersonalizedActionPlan(userProfile, topBusinessPaths[0]);
      
      // Generate motivational message
      const motivationalMessage = await this.generateMotivationalMessage(userProfile, topBusinessPaths);
  ...
  async generateDetailedAnalysis(quizData: QuizData, topPath: BusinessPath): Promise<AIAnalysis> {
    try {
...
Write in an engaging, personalized tone as if speaking directly to them.
      ...
      const fullAnalysis = response.choices?.[0]?.message?.content || this.generateFallbackAnalysisText(quizData, topPath);
      ...
      return {
        ...
        riskFactors: [
          quizData.longTermConsistency <= 2 ? 'May struggle with long-term consistency' : null,
          quizData.techSkillsRating <= 2 ? 'Technical learning curve may be challenging' : null,
          quizData.weeklyTimeCommitment <= 10 ? 'Limited time may slow initial progress' : null
        ].filter(Boolean) as string[],
        successPredictors: [
          quizData.selfMotivationLevel >= 4 ? 'High self-motivation indicates strong success potential' : null,
          quizData.longTermConsistency >= 4 ? 'Excellent consistency track record' : null,
          quizData.riskComfortLevel >= 3 ? 'Comfortable risk tolerance for entrepreneurship' : null
        ].filter(Boolean) as string[]
      };
    } catch (error) {
  ...
  private async makeOpenAIRequest(prompt: string, maxTokens: number = 200, temperature: number = 0.7): Promise<string> {
    try {
      const response = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [{ role: "user", content: prompt }],
...
- Meaningful Contribution Importance: ${quizData.meaningfulContributionImportance || 'Not specified'}/5
  ...
  private async generatePersonalizedSummary(userProfile: string, topPaths: any[]): Promise<string> {
    const prompt = `
...
Write a personalized summary that connects their specific traits to their top business match. Be encouraging and specific.
    ...
    try {
      ...
      return content || "Your unique combination of traits makes you well-suited for entrepreneurial success.";
    } catch (error) {
      return "Your unique combination of traits makes you well-suited for entrepreneurial success.";
    }
  }

  private async generateCustomRecommendations(userProfile: string, topPaths: any[]): Promise<string[]> {
    const prompt = `
...
Format as a simple list, each recommendation should be 1-2 sentences and actionable.
  ...
  private async generatePotentialChallenges(userProfile: string, topPaths: any[]): Promise<string[]> {
    const prompt = `
...
Format as a simple list, each item should be 1-2 sentences.
  ...
  private async generateSuccessStrategies(userProfile: string, topPaths: any[]): Promise<string[]> {
    const prompt = `
...
Format as a simple list, each strategy should be 1-2 sentences and actionable.
  ...
  private async generatePersonalizedActionPlan(userProfile: string, topPath: any): Promise<{
    week1: string[];
    month1: string[];
    month3: string[];
    month6: string[];
...
- Task 4
  ...
  private async generateMotivationalMessage(userProfile: string, topPaths: any[]): Promise<string> {
    const prompt = `
...
Write a motivational message that feels like it's coming from a mentor who truly understands them.
    ...
    try {
      ...
      return content || "Your unique combination of skills and drive positions you perfectly for entrepreneurial success. Trust in your abilities and take that first step.";
    } catch (error) {
  ...
  private generateFallbackAnalysis(quizData: QuizData, topPath: BusinessPath): AIAnalysis {
    return {
  ...
  private generateFallbackAnalysisText(quizData: QuizData, topPath: BusinessPath): string {
  ...
  private parseListResponse(content: string, expectedCount: number): string[] {
    ...
    while (lines.length < expectedCount) {
      lines.push("Focus on consistent daily action and continuous learning to build momentum.");
    }
  ...
  private parseActionPlan(content: string): {
    ...
    for (const line of lines) {
      if (line.toLowerCase().includes('week 1')) {
        ...
        const task = line.replace(/^[-•*]\s*/, '').replace(/^\d+\.\s*/, '').trim();
    ...
    Object.keys(sections).forEach(key => {
      ...
      while (section.length < 3) {
        section.push("Continue building your business foundation with consistent daily actions.");
      }
    });
  ...
  private generateFallbackInsights(quizData: QuizData, topPaths: BusinessPath[]): {
    personalizedSummary: string;
    customRecommendations: string[];
    potentialChallenges: string[];
    successStrategies: string[];
